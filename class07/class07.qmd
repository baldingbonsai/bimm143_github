...
ggplot(pca$x) + 
  ...
  geom_text_repel()
```
I think this plot shows that N. Ireland is very different from Wales and England and Scotland because the point is far from the other ones. I think it also shows that N.Ireland encompasses high variance. 
...
ggplot(pca$rotation) + 
  ...
  geom_col()
```
I think this plot works in conjunction with the other plot to show which foods are more common in different countries. For example, Fresh potatoes are more commonly eaten in N. Ireland, contributing more to the PC1 compared to England, Wales, and Scotland. 